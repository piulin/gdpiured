...
func set_up_sequencer(level):
	
	var thread_count = 5 if get_tag_value_of_level('STEPSTYPE', level) == 'pump-single' else 10 
	...
	var Sequencer = load('res://src/Sequencer/Sequencer.gd')
	var sequencer = Sequencer.new(args)
	
	
	# add notedata to sequencer
